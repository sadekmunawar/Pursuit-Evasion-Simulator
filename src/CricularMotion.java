 ...
 * Date: June 2021
 * 
 * Description: Strategy to move the robot in a loop.
 */
public class CricularMotion {
  ...
  public void assignCols(int numRobots, int roboID) {
    this.cols = new LinkedList<>();
    final int d = (SimulationV3.X_MAX + 1) / numRobots;
  ...
  public Coordinate getNextCoordinate(Coordinate curr) {
    ...
    final int rndmNumber = rndm.nextInt(cols.size());
    ...
    if (cols.size() == 1) {
      ...
      while (x == curr.getX()) {
        ...
        final int rndmNumber2 = rndm2.nextInt(cols.size());
    ...
    if (this.phase == Seeker.Phase.Phase2) {
      ...
      final int i = cols.get(cols.size() / 2);

      final int j = Math.abs((SimulationV3.X_MAX / 2) - i);
    ...
    if (this.phase == Seeker.Phase.Phase3) {
      if (cols.size() == 1) {
        if (Math.random() < 0.001) {
          return new Coordinate(curr.getX(), curr.getY() + 1);
        } else {
